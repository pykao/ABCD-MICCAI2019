...
df_train_age_gender = merged_train_df[['interview_age', 'gender']]
...
train_age_gender = np.array(df_train_age_gender)
...
df_valid_age_gender = merged_valid_df[['interview_age', 'gender']]
...
valid_features = np.array(df_valid)
valid_age_gender = np.array(df_valid_age_gender)
...
normalized_train_valid_features = scaler.fit_transform(train_valid_features)
...
models=[
       ...
       ExtraTreesRegressor(n_estimators=300, max_depth=13, random_state=seed, n_jobs=-1),
       ...
       [
       RandomForestRegressor(n_estimators=1000, max_depth=3, n_jobs=-1, random_state=seed),
       ExtraTreesRegressor(n_estimators=3000, max_depth=15, random_state=seed, n_jobs=-1),
       ],
       [
...
for train_idx, test_idx in skf.split(X, group_labels):
	...
	#regr = SVR(gamma='scale', C=0.25)
	#regr = Ridge(alpha=512, random_state=seed)
	...
	#regr = XGBRegressor(n_estimators=10, max_depth=7, n_jobs=-1, random_state=seed)
	...
	#regr = GradientBoostingRegressor(n_estimators=int(sys.argv[1]), max_depth=7, random_state=seed)
	...
	regr = StackNetRegressor(models, metric="rmse", folds=4, restacking=True, use_retraining=True, random_state=seed, n_jobs=-1, verbose=1)
